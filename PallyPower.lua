          ...
          for id,name in PallyPower_BlessingID do
            if name==bless then
              ...
              if (RankInfo[id] and spellRank < RankInfo[id]["rank"]) then
                ...
                if string.find(spellName, "Greater") then
                  RankInfo[id]["duration"] = 15 * 60;
                else
                  RankInfo[id]["duration"] = 5 * 60;
                end
...
function PallyPower_GetCastDuration(btn)
  local duration = 5 * 60
  local pname = UnitName("player")
  if AllPallys[pname] and AllPallys[pname][btn.buffID] and AllPallys[pname][btn.buffID]["duration"] then
    duration = AllPallys[pname][btn.buffID]["duration"]
  end
  return duration
end
...
function PallyPowerBuffButton_OnClick(btn, mousebtn)
  ...
  local duration = PallyPower_GetCastDuration(btn)
  local RecentCast = false
  if LastCast[btn.buffID..btn.classID] and LastCast[btn.buffID..btn.classID] > duration - 30 then
    RecentCast = true
  end
  ...
  -- Self-cast support: some unit frame addons / self-cast settings apply the
  -- blessing immediately after CastSpell(), so PallyPower never gets a
  -- valid SpellCanTargetUnit() match above. If the spell is no longer
  -- waiting for a target and this was our own class button, record it as
  -- a successful self-cast instead of leaving the timer at 0.
  if btn.classID == 4 and not SpellIsTargeting() then
    PP_NextScan = 1
    LastCast[btn.buffID..btn.classID] = duration;
    if not RecentCast then
      LastCastOn[btn.classID] = {}
    end
    tinsert(LastCastOn[btn.classID], "player")
    PallyPower_ShowFeedback(format(PallyPower_Casting, PallyPower_BlessingID[btn.buffID], PallyPower_ClassID[btn.classID], UnitName("player")), 0.0, 1.0, 0.0);
    TargetLastTarget()
    return
  end
